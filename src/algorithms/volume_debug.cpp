        ...
        for (uint32_t i = 0; i < samples; ++i) {
            ...
            if (x < w / 2) {
                if (y < h / 2) {
                    ...
                    Ray ray =
                        context.cam.view_ray(x, y - h / 2, w / 2, h / 2, RNG::uniform<vec2>(), RNG::uniform<vec2>());
                    L = vec3(context.scene.volume->transmittance_raymarching(ray));
                }
            } else {
                if (y < h / 2) {
                    Ray ray =
                        context.cam.view_ray(x - w / 2, y, w / 2, h / 2, RNG::uniform<vec2>(), RNG::uniform<vec2>());
                    const auto [hit, t] = context.scene.volume->sample_delta_tracking(ray);
                    const auto [bb_min, bb_max] = context.scene.volume->compute_AABB();
                    L = hit ? (bb_max - (ray.org + t * ray.dir)) / (bb_max - bb_min) : vec3(0);
                } else {
                    Ray ray = context.cam.view_ray(
                        x - w / 2, y - h / 2, w / 2, h / 2, RNG::uniform<vec2>(), RNG::uniform<vec2>()
                    );
