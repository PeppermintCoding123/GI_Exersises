...
struct ManyLights : public Algorithm {
    ...
    void init(Context& context) {
        ...
        for (uint32_t i = 0; i < NUM_VPL_PATHS; ++i) {
            ...
            trace_light_path(
                ...
                vpl_paths[i],
                light_walk,
                context.MAX_LIGHT_PATH_LENGTH,
                context.RR_MIN_PATH_LENGTH,
                context.RR_THRESHOLD
            );
        }
        size_t num_vpls = 0;
    ...
    void sample_pixel(Context& context, uint32_t x, uint32_t y, uint32_t samples) {
        samples =
            std::max(1u, samples / VPL_PATHS_PER_SAMPLE); // adjust sample count for the amount of light paths per pixel
        // init
        ...
        for (uint32_t s = 0; s < samples; ++s) {
            ...
            trace_cam_path(
                ...
                cam_path,
                cam_walk,
                context.MAX_CAM_PATH_LENGTH,
                context.RR_MIN_PATH_LENGTH,
                context.RR_THRESHOLD
            );
            // connect vertices and store result in fbo
            for (uint32_t i = 0; i < VPL_PATHS_PER_SAMPLE; ++i)
                context.fbo.add_sample(
                    x, y, connect_and_shade(context, cam_path, vpl_paths[int(RNG::uniform<float>() * vpl_paths.size())])
                );
        }
    }
