...
struct NaivePathtracer : public Algorithm {
    ...
    void sample_pixel(Context& context, uint32_t x, uint32_t y, uint32_t samples) {
        ...
        for (uint32_t i = 0; i < samples; ++i) { // samples per pixel
            vec3 L(0);
            vec3 throughput = vec3(1);
            for (uint32_t j = 0; j < context.MAX_CAM_PATH_LENGTH; ++j){ // limit recursion depth
                // TODO ASSIGNMENT4
                ...
                if (hit.is_light()){
                    if(hit.valid){
                        ...
                        // use SkyLight
                        L = context.scene.Le(ray) * throughput;
                    }
                } else {
                    ...
                    if(hit.valid){
                        if (secondary_hit.valid && secondary_hit.is_light() && dot(secondary_hit.N, -w_i) > 0 && pdf > 0){
                            throughput = throughput * brdf / (pdf);
                            L += secondary_hit.Le() * throughput * fmaxf(0.f, dot(hit.N, secondary_ray.dir)) ;
                        } 
                    } else { 
                        if (secondary_hit.valid && secondary_hit.is_light() && dot(secondary_hit.N, -w_i) > 0 && pdf > 0){
                            throughput = throughput * brdf / (pdf);
                            L += context.scene.Le(secondary_ray)* throughput * fmaxf(0.f, dot(hit.N, secondary_ray.dir)) ;
                        }
                    } 
                }
            }
