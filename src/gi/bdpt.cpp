...
void trace_cam_path(
    ...
    for (uint32_t d = 0; d < max_path_len; ++d) {
        ...
        if (!hit.is_type(BRDF_SPECULAR)) cam_path.emplace_back(hit, w_o, throughput);
        ...
        throughput *= brdf * std::abs(dot(hit.N, w_i)) / brdf_pdf;
...
void trace_light_path(
    ...
    // sample light
    auto [Le, ray, light_norm, light_pos_pdf, light_dir_pdf] =
        light->sample_Le(walk.Le_sampler.next(), walk.dir_sampler.next());
    ...
    for (uint32_t d = 1; d < max_path_len; ++d) {
        ...
        // store non-specular vertices
        if (!hit.is_type(BRDF_SPECULAR)) light_path.emplace_back(hit, w_o, throughput);
        ...
        const float num = std::abs(dot(w_o, hit.N)) * std::abs(dot(w_i, hit.Ng));
        const float denom = std::abs(dot(w_o, hit.Ng)) * std::abs(dot(w_i, hit.N));
        if (denom <= 0.f) break;
...
glm::vec3 connect_and_shade(
    ...
    // Note: to verify, when Context::MAX_LIGHT_PATH_LENGTH is set to 1, you should get idential results to a pathtracer
...
#pragma omp parallel for
    for (int p = 0; p < N; ++p) {
        ...
        // sample light source
        auto [Le, ray, light_norm, light_pos_pdf, light_dir_pdf] =
            light->sample_Le(light_pos_sampler[p], light_dir_sampler[p]);
